...
| Real robot arm integration | REAL (partial) | ArmAdapter connected to a physical 6-servo Feetech bus (`/dev/tty.usbmodem…`, IDs 1-6 @ 1 Mbaud, 6.3 ms read cycle measured); live joint telemetry streamed end-to-end | grab-incident flow on hardware not yet human-tested; torque state unknown; no arm battery/temp (sent as NOT MEASURED) | "physical arm streaming live joint state; incident capture pending physical test" |
